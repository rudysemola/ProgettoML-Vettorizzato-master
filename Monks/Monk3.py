"""
File usato per effettuare prove su monk3
"""
import sys
sys.path.append("../")
...
X, Y = load_monk("../Datasets/monks-3.train")
X_val, Y_val = load_monk("../Datasets/monks-3.test")
...
with open("../Risultati_Monk/Monk3_no_reg_medio.csv","w") as f:
    f.write("TR MSE: %3f +- %s VL MSE: %3f +- %3f TR ACC %3f +- %3f VL ACC %3f +- %3f"%(
    float(mean_err_tr[-1]), float(std_err_tr[-1]),float(mean_err_vl[-1]),float(std_err_vl[-1]),
    float(mean_error_MEE_tr[-1]),float(std_error_MEE_tr[-1]),float(mean_error_MEE_vl[-1]),
    float(std_error_MEE_vl[-1])))

with open("../Risultati_Monk/Monk3_no_reg_best.csv","w") as f:
...
with open("../Risultati_Monk/Monk3_no_reg_time.csv","w") as f:
...
plt.savefig("../Risultati_Monk/Monk3_no_reg_best.jpg")
...
plt.xlabel('epoch')
plt.legend(loc='lower right',prop={'size':12})
plt.savefig("../Risultati_Monk/Monk3_no_reg_medio.jpg")