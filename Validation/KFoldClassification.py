            ...
            for hidden in hidden_values:
                for weight in weight_values:
                    for lambd in lambda_values:
                        ...
                        """
                        Apro il file in cui salvare i risultati
                        """
                        with open("%s_eta_%s_alpha_%s_lambd_%s_hidd_%s_weight_%s.csv" % (
                          save_path_results, eta, alfa, lambd, hidden, weight),"w") as f:

                            """
                            Scrivo prime informazioni su file
                            """
                            f.write("Metrica usata per model selection = MSE\n")
                            ...
                            mean_tr_acc_matrix = np.zeros((k, n_epochs + 1))
                            mean_vl_err_matrix = np.zeros((k, n_epochs + 1))
                            mean_vl_acc_matrix = np.zeros((k, n_epochs + 1))
                            ...
                            for (idx, fold_for_vl) in enumerate(folds):
                                ...
                                mlp, mean_err_tr, std_err_tr, mean_acc_tr, std_acc_tr, mean_err_vl, std_err_vl, mean_acc_vl, std_acc_vl = run_trials(
                                    ...
                                    hidden, weight, lambd, n_trials, classification=True)

                                print(
                                    "FOLD %s: TR ERROR = %3f +- %3f TR ACCURACY = %3f +- %3f VL ERROR = %3f +- %3f VL ACCURACY = %3f +- %3f" % (
                                        idx + 1,
                                        mean_err_tr[-1], std_err_tr[-1], mean_acc_tr[-1], std_acc_tr[-1],
                                        mean_err_vl[-1], std_err_vl[-1], mean_acc_vl[-1], std_acc_vl[-1]
                                    ))

                                """
                                Scrivo su file i risultati ottenuti sul fold idx
                                """
                                f.write("FOLD %s: TR ERROR = %3f +- %3f TR ACCURACY = %3f +- %3f VL ERROR = %3f +- %3f VL ACCURACY = %3f +- %3f\n" % (
                                        idx + 1,
                                        mean_err_tr[-1], std_err_tr[-1], mean_acc_tr[-1], std_acc_tr[-1],
                                        mean_err_vl[-1], std_err_vl[-1], mean_acc_vl[-1], std_acc_vl[-1]
                                    ))
                                ...
                                mean_tr_acc_matrix[idx] = mean_acc_tr.T
                                mean_vl_err_matrix[idx] = mean_err_vl.T
                                mean_vl_acc_matrix[idx] = mean_acc_vl.T
                            ...
                            std_err_tr_list = np.std(mean_tr_err_matrix, axis=0, keepdims=True).T  # sqm (radice varianza)

                            mean_err_vl_list = np.mean(mean_vl_err_matrix, axis=0, keepdims=True).T  # Media
                            std_err_vl_list = np.std(mean_vl_err_matrix, axis=0, keepdims=True).T  # sqm (radice varianza)

                            mean_acc_tr_list = np.mean(mean_tr_acc_matrix, axis=0, keepdims=True).T  # Media
                            std_acc_tr_list = np.std(mean_tr_acc_matrix, axis=0, keepdims=True).T  # sqm (radice varianza)
                            mean_acc_vl_list = np.mean(mean_vl_acc_matrix, axis=0, keepdims=True).T  # Media
                            std_acc_vl_list = np.std(mean_vl_acc_matrix, axis=0, keepdims=True).T  # sqm (radice varianza)

                            """
                            STAMPO INFORMAZIONI COMPLESSIVE RIGUARDO ALLA CONFIGURAZIONE
                            """
                            print(
                                "INFORMAZIONI COMPLESSIVE: TR ERROR = %3f +- %3f TR ACCURACY = %3f +- %3f VL ERROR = %3f +- %3f VL ACCURACY = %3f +- %3f" % (
                                    mean_err_tr_list[-1], std_err_tr_list[-1], mean_acc_tr_list[-1], std_acc_tr_list[-1],
                                    mean_err_vl_list[-1], std_err_vl_list[-1], mean_acc_vl_list[-1], std_acc_vl_list[-1]
                                ))

                            """
                            Salvo su file i risultati complessivi della configurazione sui vari folds
                            """
                            f.write("RISULTATO FINALE KFOLD: TR ERROR = %3f +- %3f TR ACCURACY = %3f +- %3f VL ERROR = %3f +- %3f VL ACCURACY = %3f +- %3f\n" % (
                                    mean_err_tr_list[-1], std_err_tr_list[-1], mean_acc_tr_list[-1], std_acc_tr_list[-1],
                                    mean_err_vl_list[-1], std_err_vl_list[-1], mean_acc_vl_list[-1], std_acc_vl_list[-1]
                                ))
                            ...
                            plt.subplot(2, 1, 2)
                            plt.plot(mean_acc_tr_list, label='Training Error', ls="-")

                            plt.plot(mean_acc_vl_list, label='Validation Error', ls="dashed")

                            plt.fill_between(range(0, n_epochs + 1),
                                             np.reshape(mean_acc_tr_list - std_acc_tr_list, n_epochs + 1, -1),
                                             np.reshape(mean_acc_tr_list + std_acc_tr_list, n_epochs + 1, -1),
                                             color="b", alpha=0.2)

                            plt.fill_between(range(0, n_epochs + 1),
                                             np.reshape(mean_acc_vl_list - std_acc_vl_list, n_epochs + 1, -1),
                                             np.reshape(mean_acc_vl_list + std_acc_vl_list, n_epochs + 1, -1),
                                             color="orange", alpha=0.2)
                            ...
                            if mean_err_vl_list[-1] < best_mean_vl_error:
                                print(
                                    "\nTROVATO ERRORE MIGLIORE = %3f -> %3f\n" % (best_mean_vl_error, mean_err_vl_list[-1]))

                                best_mean_vl_error = mean_err_vl_list[-1]
                                best_std_vl_error = std_err_vl_list[-1]
